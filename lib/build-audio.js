...
(function main() {
  ...
  /* -------------------------------------------------- 1. VO clips -------- */
  stage('1. Trim + peak-normalise each VO clip' + (plan.vo.length ? ' (mono -> stereo at UNITY)' : ' — none configured'));
  ...
  /* --------------------------------------------------- 2. VO bus -------- */
  /* A config with no `vo` block on any scene has no narration to sum: the bus
     is skipped entirely, and the effects bus then drives the music duck. */
  const voBus = clips.length ? path.join(BUILD, 'vo_bus.wav') : null;
  let voStats = null;
  if (clips.length) {
    ...
    run('ffmpeg', args);
    voStats = clipStats(voBus);
    console.log('  vo_bus.wav  peak ' + voStats.peak.toFixed(2) + ' dBFS  rms ' + voStats.rms.toFixed(2) +
                ' dBFS  (' + duration(voBus).toFixed(3) + 's)');
  } else {
    stage('2. VO bus — no voice-over in this config, bus skipped');
    console.log('  (no `vo` block on any scene: the soundtrack is music + effects)');
  }
  ...
  /* ------------------------------------------- 3. music bed + duck ----- */
  stage('3. Music bed trimmed/faded, then ducked under the ' + (voBus ? 'VO bus' : 'sound-effect bus'));
  ...
  /* The side-chain key is whatever carries the transients: the voice bus when
     there is narration, otherwise the effects bus. With neither, the bed is
     passed through un-ducked rather than fed an empty input. */
  const duckKey = voBus || (sfxEntries.length ? sfxBus : null);
  if (duckKey) {
    run('ffmpeg', ['-y', '-hide_banner', '-loglevel', 'error', '-i', bedGain, '-i', duckKey,
    ...
    fs.copyFileSync(bedGain, ducked);
  }
  ...
  /* Probe windows for the ducking proof. With narration these bracket a spoken
     line; with none they bracket the first effect accent, which is what the bed
     now ducks under. */
  let speechWin, gapWin;
  if (clips.length) {
    speechWin = clips.length > 1
      ? [clips[1].start + 0.15, clips[1].start + 1.65]
      : [clips[0].start + 0.15, clips[0].start + 1.65];
    gapWin = [Math.max(0, clips[0].end + 0.05), Math.min(DUR, clips[0].end + 0.85)];
  } else if (sfxEntries.length) {
    const c0 = sfxEntries[0];
    speechWin = [c0.startS + 0.03, Math.min(DUR - 0.01, c0.startS + 0.43)];
    gapWin = [Math.min(DUR - 0.81, c0.startS + 0.95), Math.min(DUR - 0.01, c0.startS + 1.75)];
  } else {
    speechWin = [1.0, 1.4];
    gapWin = [2.0, 2.8];
  }
  console.log('  duck @ accent ' + rmsDb(bedGain, speechWin).toFixed(1) + ' -> ' + rmsDb(ducked, speechWin).toFixed(1) + ' dBFS RMS');
  console.log('  duck @ gap    ' + rmsDb(bedGain, gapWin).toFixed(1) + ' -> ' + rmsDb(ducked, gapWin).toFixed(1) + ' dBFS RMS');

  /* ------------------------------------------------- 4. raw mix --------- */
  stage('4. Sum ' + (voBus ? 'VO bus + ' : '') + 'ducked bed' + (sfxEntries.length ? ' + sfx bus' : ''));
  const rawMix = path.join(BUILD, '_raw_mix.wav');
  const mixIns = [], mixFc = [];
  let nMix = 0;
  if (voBus) { mixIns.push('-i', voBus); mixFc.push('[' + (nMix++) + ':a]'); }
  mixIns.push('-i', ducked); mixFc.push('[' + (nMix++) + ':a]');
  if (sfxEntries.length) { mixIns.push('-i', sfxBus); mixFc.push('[' + (nMix++) + ':a]'); }
  ...
  const voSpeech = voBus ? rmsDb(voBus, speechWin) : null;
  ...
  if (voSpeech != null) {
    ...
    console.log('  bed @ accent window          : ' + speechDucked.toFixed(1) + ' dBFS RMS (no narration in this cut)');
    console.log('  bed @ quiet  window          : ' + gapDucked.toFixed(1) + ' dBFS RMS');
  }
  ...
  if (!NO_MUX && fs.existsSync(video)) {
    ...
    console.log('  MP4 audio check : probe-window RMS ' + mp4Check.toFixed(1) + ' dBFS' +
                (voSpeech != null ? ' (source ' + voSpeech.toFixed(1) + ' dBFS)' : ''));
  }
