...
(function main() {
  ...
  /* ------------------------------------------------- 4. raw mix --------- */
  stage('4. Sum VO bus + ducked bed');
  const rawMix = path.join(BUILD, '_raw_mix.wav');
  run('ffmpeg', ['-y', '-hide_banner', '-loglevel', 'error', '-i', voBus, '-i', ducked,
    '-filter_complex', '[0:a][1:a]amix=inputs=2:normalize=0:duration=longest[o]',
