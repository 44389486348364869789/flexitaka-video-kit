...
`;
  ...
  var MARKUP = {
    transferCard: function (s) {
      ...
      return '' +
        '<div id="card">' +
          '<div class="cardHead"><div class="hdIc">' + esc(p.currency || '\u09f3') + '</div>' +
  ...
  /* --------------------------------------------------- scene 4: wallet ---- */
  function drawFlowToWallet(root, d) {
    var q = function (sel) { return root.querySelector(sel); };
    var v4a = eo(seg(d, 0.05, 0.58));
    var v4dim = 1 - 0.72 * eo(seg(d, 1.10, 1.48));
  ...
  function drawLogoReveal(root, d, s) {
    ...
    if (swT > 0 && swT < 1) {
      sweep.style.opacity = String(clamp(Math.sin(Math.PI * swT) * 1.15));
      q('#sweepBand').style.transform = 'translate3d(' + lerp(-260, W * 1.45, swT) + 'px,0,0)';
    } else {
  ...
  function render(t) {
    ...
    PLAN.scenes.forEach(function (s, i) {
      ...
      if (t <= s.start - 1 || t >= s.end + 1) return;
